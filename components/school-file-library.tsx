...
export function SchoolFileLibrary({ profile, applications, initialFiles }: { profile: Profile; applications: SchoolFileApplication[]; initialFiles: SchoolFileRecord[] }) {
  ...
  if (applications.length === 0) return <section className="panel empty-state"><h2>No school files are available.</h2><p>{profile.role === "applicant" ? "No active school application is linked to this account. Contact GHSMTA staff if you believe this is an error." : "Files will appear after a school shares them."}</p></section>;
