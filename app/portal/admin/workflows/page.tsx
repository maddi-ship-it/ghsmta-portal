...
export default async function WorkflowsAdminPage() {
  ...
  const [rulesResult, cyclesResult, applicationsResult, usersResult, digestResult, activityResult] = await Promise.all([
  ...
  for (const result of [rulesResult, cyclesResult, applicationsResult, usersResult, digestResult, activityResult]) {
  ...
  return (
    <div className="workflow-admin-grid">
      <section className="panel workflow-notification-builder">
        <div className="panel-header"><div><h2>Scheduled slot notifications</h2><p>Send automatic reminders to School Messaging, an assigned Panel Channel, the in-app inbox, or email.</p></div></div>
